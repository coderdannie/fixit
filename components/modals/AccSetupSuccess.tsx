...
const AccSetupSuccess = ({ isVisible, onClose, onProceed }: ModalProps) => {
  const handleProceed = () => {
    if (onProceed) {
      onProceed();
    }
      ...
      <View className="flex-1 bg-black/50 justify-center items-center px-5">
        <View className="bg-[#F5F5F5] rounded-[32px] p-10 items-center w-full max-w-[450px] shadow-2xl">
          ...
          <Text className="text-base text-[#666666] text-center leading-6 mb-8 px-2">
            You are now ready to start getting jobs and keeping vehicles in top
            shape with FIXIT.
          </Text>
...
export default AccSetupSuccess;
